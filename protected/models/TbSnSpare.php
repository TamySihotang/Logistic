...
class TbSnSpare extends CActiveRecord
{
	...
	public function searchSpare($id)
	{
		// @todo Please modify the following code to remove attributes that should not be searched.
		...
		$criteria->compare('id_sn',$this->id_sn);
                $criteria->compare('id_office',$this->id_office);
		$criteria->compare('id_spare',$id, true);
		$criteria->compare('id_status_spare',$this->id_status_spare);
		$criteria->compare('serial_number',$this->serial_number,true);
	...
	public function searchOffice($id)
	{
		...
		$criteria->compare('id_sn',$this->id_sn);
                $criteria->compare('id_office',$id, true);
		$criteria->compare('id_spare',$this->id_spare);
		$criteria->compare('id_status_spare',$this->id_status_spare);
		$criteria->compare('serial_number',$this->serial_number,true);
	...
	public static function model($className=__CLASS__)
	{
		return parent::model($className);
         ...
         public function getOffice(){
             $model = new TbOffice;
        $spareoffice = TbSnSpare::model()->findAll($model->id_office);
            foreach ($spareoffice as $value) {
                $sparelist = TbSpare::model()->findByAttributes(array('id_spare' => $value->id_spare));
//                $spare = Spare::model()->findByAttributes(array('id_spare' => $sparelist->id_spare));
                if($value->id_office == $model->id_office){
                    echo $sparelist->aliasname.'<br>';
                }
            }
    }
    public function getStockOffice($id) {
        //$id = Yii::app()->user->Id;
        $spareoffice = TbSnSpare::model()->findAll();
        //$model = StatusPermohonancuti::model()->findAll($criteria);
        $counter = 0;
        foreach ($spareoffice as $value) {
            if ($value->id_office == $id && $value->id_spare == $id) {
                $counter++;
            }
        }
        return $counter;
